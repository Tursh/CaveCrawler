...
{
	timer_ += CGE::Utils::TPSClock::getCooldown();
	//logInfo(timer_);
}
...
void CC::Weapon::shoot(CC::Entities::Player *player, CC::World *world)
{
	...
	CC::Entities::SharedProjectile projectile = std::make_shared<Entities::Projectile>(
	...
	projectile->setPositions(
			...
			glm::vec3(distribution(random_data),distribution(random_data),distribution(random_data))
			* (1 / precision_));
